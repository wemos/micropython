...
def dis_page():
    ...
    if(page_state==0):
        tft.text(font16, "IMU&RGB", 0, 0, st7789.GREEN, st7789.BLACK)
    elif(page_state==1):
        tft.text(font16, "IR", 0, 0, st7789.GREEN, st7789.BLACK)
        tft.text(font, "OK - SEND", 0, 16, st7789.YELLOW, st7789.BLACK)
    elif(page_state==2):
        tft.text(font16, "WIFI", 0, 0, st7789.GREEN, st7789.BLACK)
...
def dis_wifi():
    ...
    for n in nets:
        if(n[0]!=b''and n[0]!=b' '):
            tft.text(font, "{:<13s}{:>3d}".format(n[0][0:12],n[3]), 0, 16+16*i, st7789.YELLOW, st7789.BLACK)
            i=i+1
            if(i>6):
                break
...
def test_imu_rgb():
    ...
    while(1):
        for led in rgb_led_state:
            ...
            tft.text(font, "Acc_X: {:>9.3f}".format(ax), 0, 16, st7789.YELLOW, st7789.BLACK)
            tft.text(font, "Acc_Y: {:>9.3f}".format(ay), 0, 32, st7789.YELLOW, st7789.BLACK)
            tft.text(font, "Acc_Z: {:>9.3f}".format(az), 0, 48, st7789.YELLOW, st7789.BLACK)
            tft.text(font, "Gyr_X: {:>9.3f}".format(gx), 0, 64, st7789.YELLOW, st7789.BLACK)
            tft.text(font, "Gyr_Y: {:>9.3f}".format(gy), 0, 80, st7789.YELLOW, st7789.BLACK)
            tft.text(font, "Gyr_Z: {:>9.3f}".format(gz), 0, 96, st7789.YELLOW, st7789.BLACK)
